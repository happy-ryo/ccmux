...
- **中央多行の IME composition overlay** — `Alt+Enter` で送信、`Enter` で改行、候補窓がキャレットに吸着 (ccmux-fork 独自)
- **ペイン凍結 + 周期 catch-up** (`--ime-freeze-panes` + `--ime-overlay-catchup-ms`) で IME 入力中のちらつきを抑制 (ccmux-fork 独自)
