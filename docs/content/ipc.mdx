...
<Callout type="info">
  エンドポイントは Unix socket (macOS / Linux) または Named Pipe (Windows)。same-user にのみアクセス可能で、`CCMUX_SOCKET` / `CCMUX_TOKEN` 環境変数で識別する。ccmux pane 内で起動した子プロセスはこれらを継承するので、`claude` / `bash` / スクリプトから透過的に呼び出せる。
</Callout>
