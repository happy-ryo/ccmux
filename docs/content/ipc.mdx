...
ccmux は起動中のインスタンスを外部プロセスから制御できる **local-only IPC** を持つ。スクリプト / エージェント / 別の Claude Code セッションから pane 操作 / 画面取得 / ライフサイクル監視が行える。

import { Callout } from 'nextra/components'

<Callout type="info">
  エンドポイントは Unix socket (macOS / Linux) または Named Pipe (Windows)。access control は **OS レベルの socket パーミッション** に委ねられ、same user でのみ到達できる (詳細は「セキュリティモデル」参照)。`CCMUX_SOCKET` / `CCMUX_TOKEN` 環境変数はクライアントが **エンドポイントを見つけるための指標**で、ccmux pane 内で起動した子プロセス (claude / bash / スクリプト) はこれらを継承して透過的にサブコマンドを呼び出せる。
</Callout>
...
`x` / `y` / `width` / `height` は「最後に描画された時点」での pane の矩形
（ターミナル左上 = `(0, 0)`、単位はセル）。まだ一度も描画されていない
場合や layout 更新の直前は `0` が入ることがある。既存 consumer は新
フィールドを無視しても動作する。
...
ccmux send --focused $'\x1b'  # Esc を送る
```
...
IPC 応答のエラーは `[<code>] <message>` 形式で stderr に出る (ccmux 0.5.7+)。code は wire ABI として安定しており、substring match ではなく code で分岐するのが推奨。

| Code | 意味 |
|---|---|
| `shutting_down` | ccmux 本体がシャットダウン中。クライアント側は即停止 |
...
| `split_refused` | `ccmux split` が MAX_PANES / 最小サイズ制約で拒否 |
...
`ccmux events` 購読は server 側で half-close 検知される。クライアントが read 側を閉じても、server 側の 30 秒おきの heartbeat write が失敗した時点で subscriber slot が解放される — quiet な workspace でも stale subscriber が残らない設計。

## セキュリティモデル

same-user 前提の local IPC で、秘匿通信用ではない。同一ユーザーで動く他プロセスは `/proc/<pid>/environ` 等から `CCMUX_TOKEN` を読める。OS レベルの user isolation が信頼境界。

`CCMUX_TOKEN` は「PID 再利用で別 ccmux に誤接続しないための session token」として機能する。
