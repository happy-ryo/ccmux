...
| **`Alt+Enter`** | 送信 (全環境対応、macOS `Option+Return` 含む)。WSL2 / Windows Terminal ではホストが *Toggle Fullscreen* に割り当てて飲み込むため renga まで届きません。代わりに `Ctrl+Enter` を使ってください (Issue #226)。 |
