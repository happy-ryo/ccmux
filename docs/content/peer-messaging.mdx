...
<Callout type="info">
  Claude 側は `<channel source="renga-peers">...</channel>` タグで受信するので、ユーザー入力と区別できます。Codex 側は `check_messages` による pull 受信なので、割り込み型の `<channel>` にはなりません。
</Callout>
...
| `check_messages` | pull 型 client の受信箱を drain します。Claude では主にフォールバックですが、Codex ではこれが主受信経路なので、各ターン開始時と長作業の節目で呼ぶ前提です。 |
