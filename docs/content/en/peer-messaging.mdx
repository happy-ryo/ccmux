...
<Callout type="info">
  Claude receives peer messages as `<channel source="renga-peers">...</channel>` tags, which keeps them distinct from user input. Codex uses a pull-based inbox via `check_messages`, so it does not get interrupt-style `<channel>` delivery.
</Callout>
