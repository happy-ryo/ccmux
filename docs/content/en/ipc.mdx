...
`x` / `y` / `width` / `height` are the pane's last known on-screen
rectangle in cells (terminal origin = `(0, 0)`). They fall back to `0`
before the first render, so treat zeros as "not yet drawn" rather than
"zero-sized pane". Older clients that don't know about these fields can
safely ignore them.
