...
<Callout type="info">
  The endpoint is a Unix socket (macOS / Linux) or a Named Pipe (Windows). Access is scoped to the same OS user via `CCMUX_SOCKET` / `CCMUX_TOKEN` environment variables. Any process spawned inside a ccmux pane (`claude`, a shell, a script) inherits them and can call ccmux subcommands transparently.
</Callout>
