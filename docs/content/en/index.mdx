...
- **Centered multi-line IME composition overlay** — `Alt+Enter` sends, `Enter` inserts a newline, the host IME candidate window anchors to the caret (ccmux-fork exclusive)
- **Pane freeze + periodic catch-up** (`--ime-freeze-panes` + `--ime-overlay-catchup-ms`) silences caret flicker during IME composition (ccmux-fork exclusive)
