...
If you want Claude Code panes to talk to each other or control panes from inside Claude, register renga's MCP server once:

```bash
renga mcp install
```

That exposes the built-in `renga-peers` channel and pane-control tools such as `list_panes`, `spawn_claude_pane`, `inspect_pane`, and `send_keys` inside Claude Code.
...
### 1. Start Claude Code the peer-aware way
...
After `renga mcp install`, a Claude pane can call tools like `list_peers`, `send_message`, `list_panes`, `spawn_claude_pane`, `inspect_pane`, `send_keys`, and `poll_events`. That is the core renga workflow: one pane can launch workers, inspect their prompts, and coordinate them without leaving the conversation.
