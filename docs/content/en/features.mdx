...
| **`Alt+Enter`** | Send (works everywhere, including macOS `Option+Return`). On WSL2 / Windows Terminal the host emulator binds this chord to *Toggle Fullscreen* and consumes it before renga sees it — use `Ctrl+Enter` instead (Issue #226). |
