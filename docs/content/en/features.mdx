...
ccmux paints a single-row **composition overlay** at the bottom of the screen to give the host IME a stable anchor, and commits the confirmed text to the pane via bracketed paste.

Three modes selected via the `[ime]` config section or `--ime` flag:

| Mode | Behavior |
|---|---|
| `hotkey` (default) | `Ctrl+;` opens the overlay. Enter commits, Esc cancels |
| `off` | `Ctrl+;` is swallowed, overlay never opens. Opt-out for environments where the problem doesn't exist |
| `always` | The overlay stays open on focused Claude panes so JP IME can compose from the first keystroke |
...
The `--ime hotkey|off|always` CLI flag overrides the file. Precedence is **CLI > config > default (`hotkey`)**. See the README Configuration section for more details.

Windows' conpty layer flickers the hardware caret during ratatui's diff paint; ccmux issues `cursor::Hide` while the overlay is open (`#[cfg(windows)]`) to mitigate. Not a full fix — see Issues #37 / #38.
...
| `Ctrl+;` | Open the IME overlay (mode = `hotkey` / `always`) |
...
| `Ctrl+;` | IME overlay (mode hotkey/always) | `--ime off` swallows the keystroke |
