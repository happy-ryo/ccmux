...
In alt-screen TUIs (Claude Code `/tui fullscreen`, `vim`, `lazygit`, …) the wheel is forwarded to the app instead, so the app's own scrolling takes over. See [Keybindings > Mouse Wheel](/docs/en/keybindings#mouse-wheel) for the full table.
...
| Scroll in an alt-screen TUI pane | Forward the wheel event to the app |
