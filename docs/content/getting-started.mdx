...
Claude Code ペイン同士の会話や、Claude からのペイン操作を使いたい場合は、renga の MCP サーバを一度登録してください。

```bash
renga mcp install
```

これで Claude Code 内から、組み込みの `renga-peers` チャネルと `list_panes` / `spawn_claude_pane` / `inspect_pane` / `send_keys` などのペイン制御ツールが使えるようになります。
...
`renga mcp install` 済みなら、Claude ペインは `list_peers` / `send_message` / `list_panes` / `spawn_claude_pane` / `inspect_pane` / `send_keys` / `poll_events` などを呼べます。これが renga の中核で、1 つの Claude がワーカー起動、画面確認、対話プロンプト応答まで会話の中で完結できます。

詳しくは [Claude Code ペイン同士のメッセージング](/ja/peer-messaging) を参照してください。
