...
| 値 | 動作 |
|-------|----------|
| `hotkey` (デフォルト) | `Ctrl+;` で現在のペインに overlay を開きます。`Ctrl+;` を食ってしまうターミナル (WSL + Windows Terminal、Linux 上の VS Code ターミナルなど) では `Alt+;` / `Alt+I` もフォールバックとして受け付けます。 |
...
| `Ctrl+;` / `Alt+;` / `Alt+I` | IME overlay を開く (上記参照)。`Ctrl+;` が効かないターミナル (WSL + Windows Terminal、Linux 上の VS Code ターミナル、一部の tmux 設定など) では `Alt+;` か `Alt+I` をフォールバックとして使えます。 |
| `Ctrl+Q` | ccmux 終了 |
