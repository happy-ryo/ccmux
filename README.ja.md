...
npm が新しくキャッシュも素直なら `npm update -g` で上がります。スキップされたように見える場合は `@latest` 付きの `install` で確実に最新を引いてください。`ccmux --version` で確認し、[最新リリース](https://github.com/happy-ryo/ccmux/releases/latest) と照合できます。
