...
- **同じタブ内の Claude Code 同士のメッセージング** — 同タブの Claude Code インスタンスが MCP channel 経由で会話できる。peer からのメッセージはユーザー入力とは別の `<channel source="ccmux-peers">` タグとして届く ([詳細](#claude-code-ペイン間のメッセージング))
...
## Claude Code ペイン間のメッセージング

同じ ccmux タブの中にいる複数の Claude Code インスタンスが、お互いにメッセージを送り合えるようにします。片方の Claude が「これを調べて」と頼んだり、失敗したテストの原因を他方にハンドオフしたり、ユーザーが手動でコピペを挟まなくてもコーディネーションできます。peer から届くメッセージは `<channel source="ccmux-peers">` タグで到着するので、**ユーザー入力とは明確に区別された形で Claude Code のコンテキストに入ります**。

> **[`claude-peers-mcp`](https://github.com/happy-ryo/claude-peers-mcp) との違い** — ツールのシグネチャはほぼ同じですが、スコープの権威が違います。`claude-peers-mcp` は `cwd` / `git_root` / `PID` の組み合わせからピアを推論するヒューリスティック、ccmux-peers は **ccmux タブそのもの** を authoritative なスコープにします (ユーザーが「このタブに 2 つ Claude 並べた」という物理配置 = 明確な意図)。チャンネル名が違う (`server:ccmux-peers` と `server:claude-peers`) ので、同じ Claude Code に両方インストールしていても衝突しません。
...
現在走っている `ccmux` バイナリを `ccmux-peers` MCP server として Claude Code のユーザー設定に登録します。再実行しても idempotent、ccmux をアップグレードしたあとにパスを更新したいときは `--force` を付けてください。`ccmux mcp uninstall` で解除、`ccmux mcp status` で状態確認できます。

### 使い方 — peer channel 付きで Claude Code を起動

peer メッセージングは MCP の experimental channel 機能に乗るので、Claude Code 起動時に `--dangerously-load-development-channels server:ccmux-peers` フラグが必要です。毎回手で打たなくてよいように ccmux 側で 2 つのショートカットを用意しています：

- **`Alt+P`** — フォーカス中のペインに `claude --dangerously-load-development-channels server:ccmux-peers ` を挿入 (末尾スペース、**Enter は打たれない**)。確認・追記・そのまま Enter でも OK。shell を問わず動きます。
- **`ccmux split --role claude`** / **`ccmux new-tab --role claude`** — 新しいペインを開いて自動で上のフラグ付き Claude Code を起動します。明示的に `--command` を渡した場合はそちらが優先されるので、カスタム起動のための逃げ道は残ります。
...
Claude B の次ターンの context に `<channel source="ccmux-peers">src/app.rs の handle_split を読んで...</channel>` タグが現れ、Claude B は「peer からの依頼」として認識 (タグの source 属性で区別) → 要件を実行 → 同じ仕組みで返信します。

**提供ツール一覧:**

| Tool | 効果 |
|---|---|
| `list_peers` | 呼び出し元と同じタブの他ペインを返す。自分自身は除外。 |
| `send_message(to_id, message)` | 同タブの peer に numeric id または stable name で送信。タブ外宛は silent no-op (他タブを列挙されないようにするため)。 |
| `check_messages` | 手動 drain。通常は channel push が一次経路、再確認したいときの補助。 |
| `set_summary` | v1 では stub。ccmux はペイン名 / role を summary 代替として使います。 |

### 困ったとき

- **`list_peers` が "ccmux not reachable from this Claude Code instance" を返す** — Claude Code が ccmux の外で起動されているか、env を継承していない状態。ccmux 内のペインで `Alt+P` または `ccmux split --role claude` から起動し直してください。
- **peer メッセージが `<channel>` タグとしてレンダリングされない** — `--dangerously-load-development-channels server:ccmux-peers` を付け忘れています。素の `claude` を打つより `Alt+P` を使ったほうが事故りません。
- **ccmux をアップグレードした** — `ccmux mcp install --force` で登録済みバイナリパスを更新してください。
...
| `Alt+S` | ステータスバー表示切替 |
| `Alt+P` | フォーカス中のペインに peer 対応の Claude Code 起動コマンドを挿入 ([詳細](#claude-code-ペイン間のメッセージング)) |
| `Ctrl+F` | ファイルツリー表示切替 |
