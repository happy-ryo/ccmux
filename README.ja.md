...
この違いは位置づけに効きます。renga 単体では「ローカルなマルチエージェント開発のための鋭い基盤」であり、`claude-org` と組み合わせると「より完全な local AgentOps stack」の一部になります。考え方としては **renga が execution fabric、claude-org がその上の reference operating system** です。
...
- **mixed-client peer メッセージング** — 同じタブに並べた Claude Code と Codex が `renga-peers` で協調できる。Claude は `<channel source="renga-peers">` タグで push 受信し、Codex は renga がペイン経由で配送する ([詳細](#claude-code-と-codex-ペイン間のメッセージング))
...
4. **複数行のドラフトをそのまま書ける。** `Enter` は改行、送信は `Alt+Enter` (macOS は `Option+Return`)、Windows Terminal / wezterm / VS Code なら `Ctrl+Enter` でも送れます。WSL2 / Windows Terminal ではホスト側が `Alt+Enter` を *Toggle Fullscreen* に割り当てて飲み込んでしまうため、`Ctrl+Enter` を使ってください (Issue #226)。詳しいキーマップは次節を参照。
...
| `Alt+Enter` | 送信して overlay を閉じる (主要ターミナルでのカノニカルな commit、macOS では `Option+Return` — Option が効かない場合は [macOS: Option をメタキーにする](#macos-option-をメタキーにする) を参照)。WSL2 / Windows Terminal ではホストが *Toggle Fullscreen* に bind しているため renga まで届かない。代わりに `Ctrl+Enter` を使ってください (Issue #226)。 |
| `Ctrl+Enter` | 送信 (Windows Terminal / wezterm / VS Code / 多くの Linux ターミナル)。WSL2 / Windows Terminal ではホストが `Alt+Enter` を奪うため、こちらが推奨。renga は `Ctrl+J` も commit として受け付けます。これは extended-key reporting が無効なホストが Ctrl+Enter を素の LF バイト (0x0A) として送ってきた場合の同義扱いです。 |
...
同じ renga タブに並べた Claude Code と Codex のペイン同士が、お互いにメッセージを送り合えるようになります。片方のエージェントに「これを調べておいて」と頼んだり、失敗したテストの原因追いをもう片方に引き継いだりといった連絡を、ユーザーが手でコピペして中継しなくても進められます。Claude は `<channel source="renga-peers">` タグで受け取り、Codex には renga が `check_messages` を促す nudge を PTY 経由で送り、実本文は MCP inbox から読ませます。
...
| `Alt+S` | ステータスバー表示切替 |
| `Alt+P` | フォーカス中のペインにメッセージング対応の Claude Code 起動コマンドを入力 ([詳細](#claude-code-ペイン同士のメッセージング))。フォーカス中のペインが alt-screen モード (vim / less / lazygit / 起動中の Claude / Codex の TUI 等) かペインタイトルに "claude" を含む場合は仕様として silently no-op になります — 起動中の TUI にコマンドのバイト列がキーストロークとして注入されるのを防ぐためです。シェルプロンプトのペインにフォーカスを移してから再度押してください (Issue #234)。 |
| `Ctrl+F` | ファイルツリー表示切替 |
