...
> **これは [Shin-sibainu/ccmux](https://github.com/Shin-sibainu/ccmux) のフォークです。** 上流を定期的に取り込みつつ、独自機能を先行して足しています。npm では別パッケージ `renga-fork` として配布しています。ブランチの使い分けは [`BRANCHING.md`](./BRANCHING.md) を参照してください。
...
> 上流の `renga-cli` を入れている場合は: `npm uninstall -g renga-cli && npm install -g renga-fork`
