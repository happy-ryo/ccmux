...
| `Alt+Enter` | 送信して overlay を閉じる (主要ターミナル全般で動作、macOS では `Option+Return` — Option が効かない場合は [macOS: Option をメタキーにする](#macos-option-をメタキーにする) を参照)。WSL2 / Windows Terminal ではホストが *Toggle Fullscreen* に bind しているため renga まで届かない。代わりに `Ctrl+Enter` を使ってください (Issue #226)。 |
