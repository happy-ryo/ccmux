...
| `always` | Claude pane にフォーカスが乗ると overlay を自動起動し続ける。JP IME が最初のキーから変換可能になる。空のバッファで `Esc` (または `Ctrl+C`) を押すと overlay を解除し pane を直接操作可能 — キャンセルキーは pane に転送されるので Claude の Esc-to-interrupt も動く。別の pane に移ってから戻ると再オープン。解除済み overlay で印字可能キーを押すと半角ショートカットとして auto-open する。スクロール中やシェルペインでは auto-open しない。**トレードオフ:** overlay を開いている間、ccmux の pane 操作ショートカット (Ctrl+D 分割、Ctrl+Left/Right フォーカス切替、Alt+Left/Right タブ移動など) は発火しない。先に overlay を閉じてから使うこと。この摩擦が嫌なら `hotkey` のまま、必要なときだけ Ctrl+; を押す運用で十分。 |

CLI フラグ `--ime hotkey|off|always` は単発実行で config ファイルを上書き。優先順位は **CLI > config ファイル > デフォルト**。

### JP / CJK IME ユーザー向け推奨セットアップ

日本語 (その他 IME が重要な言語) で Claude にプロンプトを書くことが多いなら、以下の 3 点セットで起動するのがおすすめ:
...
1. **Overlay が自動で開く。** Claude pane にフォーカスが乗った瞬間、中央に多行の composition box が表示されます。ホストターミナルの IME 候補窓が box 内のキャレットに吸着するので、長い日本語のフレーズが変換中に画面を飛び回るのが止まります (Issue #25)。
...
5. **緊急脱出。** 空バッファで `Esc` を押せば overlay が閉じるので ccmux の pane 操作ショートカット (`Ctrl+D` 分割、`Ctrl+Left/Right` フォーカス切替など) を使えます。別の pane に移ってから戻れば再オープン。

always-open の挙動が鬱陶しいと感じたら、`mode = "always"` を `mode = "hotkey"` に切り替えるだけで、必要なときに `Ctrl+;` で開く運用になります。残り 2 つのフラグは引き続きちらつき抑制に効きます。
...
| `Esc` / `Ctrl+C` | キャンセル。空バッファで overlay を閉じる。Always モードで非空バッファなら 1 回目はクリアのみ、2 回目で解除 |
| `←` `→` `↑` `↓` | カーソル移動 |
| `Home` / `End` | 現行行の先頭 / 末尾 |
